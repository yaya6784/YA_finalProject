...
# מקבלת פקודה ושדות, בודקת שהמבנה תקין, ומחזירה הודעה בפורמט CMD|field|field.
...
# מקבלת הודעת טקסט, בודקת את מבנה הפרוטוקול, ומחזירה פקודה ורשימת שדות.
def parse_msg(text):
...
# מקבלת שם שחקן, ומחזירה True רק אם השם לא שובר את הפרוטוקול.
...
# מקבלת שתי נקודות, ומחזירה את המרחק ביניהן.
def distance(x1, y1, x2, y2):
    return math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2)


# לא מקבלת כלום, ומחזירה מיקום אוכל רנדומלי במפה.
def new_food():
    return [random.randint(20, WORLD_W - 20), random.randint(20, WORLD_H - 20)]


# מקבלת socket וטקסט, מצפינה אם החיבור מאובטח ושולחת עם tcp_by_size.
...
# מקבלת socket וקוד שגיאה, ושולחת ללקוח הודעת שגיאה מסודרת.
def send_error(sock, code):
    send_packet(sock, build_msg(code, ERROR_TEXT.get(code, "error")))


# מקבלת socket, קוראת ממנו הודעה, מפענחת אם צריך ומחזירה טקסט.
...
# מקבלת socket, מבצעת החלפת מפתח RSA/AES ומחזירה True אם הצליחה.
def handshake(sock):
...
# מקבלת מילון של שחקן, ומאפסת לו מיקום וגודל התחלתי.
...
# מקבלת id ושם תקין, ומחזירה מילון שמייצג שחקן חדש.
...
# לא מקבלת כלום, מתחילה משחק חדש לכל מי שמוכן ומאפסת אוכל/מנצח.
def start_game():
...
# לא מקבלת כלום, בודקת אם כל השחקנים מוכנים ואז מתחילה משחק.
def check_start():
    if phase != 0:
        return
...
# לא מקבלת כלום, בודקת אם נשאר שחקן אחד ומעבירה למסך ניצחון.
def check_winner():
...
# לא מקבלת כלום, מחזירה את כל השחקנים כמחרוזת אחת לשליחה ללקוח.
def encode_players():
    text = ""
    for player in players.values():
        color = player["color"]
...
# לא מקבלת כלום, מכניסה לכל לקוח הודעת STATE עם מצב המשחק הנוכחי.
def queue_state():
...
# לא מקבלת כלום, מעדכנת תנועה של כל השחקנים שבמשחק.
def update_movement():
    for player in players.values():
...
# לא מקבלת כלום, בודקת אכילת אוכל ומגדילה שחקנים שאכלו.
def update_food():
    for player in players.values():
        if not player["ingame"]:
            continue
...
# לא מקבלת כלום, בודקת אם שחקן גדול אכל שחקן קטן.
...
# לא מקבלת כלום, מעדכנת את המשחק אם הוא רץ.
def update_game():
    if phase != 1:
        return
    update_movement()
...
# לא מקבלת כלום, רצה ברקע ומעדכנת את המשחק כמה פעמים בשנייה.
def game_loop():
    global foods
    foods = [new_food() for _ in range(FOOD_AMOUNT)]
    while True:
...
# מקבלת שדות TRGET, בודקת אותם, ומחזירה x,y או None אם הם לא תקינים.
...
# מקבלת id של שחקן וטקסט מהלקוח, ומעדכנת את מצב השחקן לפי הפקודה.
...
# מקבלת socket ו-id, מוחקת את השחקן ומנקה את החיבור שלו.
def cleanup_client(sock, player_id):
    with lock:
        was_ingame = players.get(player_id, {}).get("ingame", False)
...
# מקבלת socket/id/כתובת, מטפלת בלקוח אחד בלולאה משלו.
def handle_client(sock, player_id, addr):
    try:
        ...
        if cmd not in (LOGIN, SIGNP):
            ...
            return
        if len(fields) != 1 or not is_good_name(fields[0]):
            send_error(sock, ERR01)
            return

        with lock:
            clients[player_id] = sock
            players[player_id] = make_player(player_id, fields[0])
...
# לא מקבלת כלום, פותחת שרת ומייצרת thread חדש לכל לקוח שמתחבר.
def main():
